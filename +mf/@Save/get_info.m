    ...
    for i = 1 : length(fields)
        ...
        for j = 1 : size(obj.model.Vs.(field), 2)
            ...
            if strcmp(field, 'toroidal')
                ...
                tof_inner = obj.model.to_time(v.tof.inner);
                tof_outer = obj.model.to_time(v.tof.outer);
                info = [info, ...
                'harm. frequencies: ', ...
                ...
                'ampl. protocol  with ', v.U.shape, ' shape : ', ...
                ...
                'time of flight   : ',  newline, ...
                'inner edge t = [', num2str(tof_inner(1)), ', ', num2str(tof_inner(2)), '] s', newline, ... 
                'outer edge t = [', num2str(tof_outer(1)), ', ', num2str(tof_outer(2)), '] s', newline, ... 
                 newline];
            end

            if strcmp(field, 'vertical')
                W = obj.model.to_omega(v.W)/(2*pi);
                t = obj.model.to_time(v.U.t);
                rlim = ['[', num2str(v.rlim(1), '%.2f'), ', ', num2str(v.rlim(2), '%.2f'), '] lr'];

                phi = 'pi*[';
                for n_index = 1 : v.n
                    phi = [phi, num2str(v.phi(n_index)/pi, '%.2f'), ','];
                end
                phi = [phi(1 : end-1), ']'];

                info = [info, ...
                       'amount of barriers: n = ', num2str(v.n), ...
                       ' with positions phi = ', phi, newline, ...
                       'width = ', num2str(v.width, '%.2f'), ' lr', newline, ...
                       'rlim = ', rlim, newline, ...
                       'rotating frequency: ', ...
                       'W = 2*pi*', num2str(W, '%.3g'), ' Hz', newline, ...
                       'ampl. protocol with ', v.U.shape, ' shape : ', ...
                       'U = ', num2str(v.U.max, '%.3g'), ' hwr', ', ', newline, ...
                       't = [', num2str(t(1)), ', ', num2str(t(2)), ', ', num2str(t(3)), ', ', num2str(t(4)), '] s', newline, ... 
                       newline];
            end
